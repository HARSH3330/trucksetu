...
from app.models import Booking, BookingAllocation, Conversation, ConversationParticipant, Message, Notification, NotificationPreference, ProviderProfile, User
...
async def _booking_participants(db:AsyncSession,booking:Booking)->dict[uuid.UUID,str]:
    ...
    provider_users = await db.scalars(select(ProviderProfile.user_id).join(BookingAllocation, BookingAllocation.provider_id == ProviderProfile.id).where(BookingAllocation.booking_id==booking.id))
    for user_id in provider_users:result[user_id]="provider"
    return result


@router.get("/notifications")
async def list_notifications(unread_only:bool=False,db:AsyncSession=Depends(get_db),user:User=Depends(current_user))->dict[str,object]:
    ...
    unread=await db.scalar(select(func.count(Notification.id)).where(Notification.user_id==user.id,Notification.read_at.is_(None)))
...
async def mark_notification_read(notification_id:uuid.UUID,db:AsyncSession=Depends(get_db),user:User=Depends(current_user))->dict[str,bool]:
    item=await db.scalar(select(Notification).where(Notification.id==notification_id,Notification.user_id==user.id).with_for_update())
...
async def update_preferences(payload:NotificationPreferenceUpdate,db:AsyncSession=Depends(get_db),user:User=Depends(current_user))->dict[str,object]:
    item=await db.get(NotificationPreference,user.id)
    if item is None:item=NotificationPreference(user_id=user.id);db.add(item)
...
async def list_messages(conversation_id:uuid.UUID,db:AsyncSession=Depends(get_db),user:User=Depends(current_user))->list[dict[str,str]]:
...
async def send_message(conversation_id:uuid.UUID,payload:MessageCreate,db:AsyncSession=Depends(get_db),user:User=Depends(current_user))->dict[str,str]:
    ...
    try:ensure_chat_allowed(booking.status if booking else "cancelled",str(user.id),{str(x) for x in participants})
    except PermissionError as exc:raise HTTPException(status_code=403,detail=str(exc)) from exc
    item=Message(conversation_id=conversation.id,sender_id=user.id,**payload.model_dump());db.add(item);await db.flush()
    for user_id in participants:
        if user_id!=user.id:db.add(Notification(user_id=user_id,event_type="chat.message",title="New booking message",body=payload.body[:180],entity_type="conversation",entity_id=conversation.id,data={"booking_id":str(conversation.booking_id)}))
