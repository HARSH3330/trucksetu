...
from app.models import ApplicationSetting, AvailableRoute, Booking, BookingAllocation, Cancellation, CapacityReservation, Dispute, DisputeMessage, Payment, ProviderProfile, Review, SafetyReport, Trip, User
...
async def _participants(db: AsyncSession, booking: Booking) -> set[uuid.UUID]:
    providers = set(await db.scalars(select(ProviderProfile.user_id).join(BookingAllocation, BookingAllocation.provider_id == ProviderProfile.id).where(BookingAllocation.booking_id == booking.id)))
...
async def create_review(booking_id: uuid.UUID, payload: ReviewCreate, db: AsyncSession = Depends(get_db), user: User = Depends(current_user)) -> dict[str, object]:
    ...
    if user.id not in participants or payload.target_id not in participants or user.id == payload.target_id:
...
async def raise_dispute(booking_id: uuid.UUID, payload: DisputeCreate, db: AsyncSession = Depends(get_db), user: User = Depends(current_user)) -> dict[str,str]:
    booking=await db.scalar(select(Booking).where(Booking.id==booking_id).with_for_update())
    if booking is None:raise HTTPException(status_code=404,detail="Booking not found")
    if user.id not in await _participants(db,booking):raise HTTPException(status_code=403,detail="Only booking participants can raise a dispute")
    dispute=Dispute(booking_id=booking.id,raised_by=user.id,category=payload.category,description=payload.description)
    dispute_message=DisputeMessage(sender_id=user.id,message=payload.description,attachment_keys=payload.attachment_keys)
...
async def add_dispute_message(dispute_id:uuid.UUID,payload:DisputeMessageCreate,db:AsyncSession=Depends(get_db),user:User=Depends(current_user))->dict[str,str]:
    ...
    if booking is None or user.id not in await _participants(db,booking):raise HTTPException(status_code=403,detail="Only dispute participants can add messages")
    item=DisputeMessage(dispute_id=dispute.id,sender_id=user.id,**payload.model_dump());db.add(item);await db.flush();return {"id":str(item.id)}


@router.post("/bookings/{booking_id}/cancel", status_code=status.HTTP_201_CREATED)
async def cancel_booking(booking_id:uuid.UUID,payload:CancellationCreate,db:AsyncSession=Depends(get_db),user:User=Depends(current_user))->dict[str,str]:
    booking=await db.scalar(select(Booking).where(Booking.id==booking_id).with_for_update())
    if booking is None:raise HTTPException(status_code=404,detail="Booking not found")
    ...
    if booking.capacity_reservation_id:
        ...
        if reservation and reservation.status == "confirmed":
            ...
            if route:
                release_reservation(route, reservation, "released")
...
async def resolve_dispute(dispute_id:uuid.UUID,resolution:str,db:AsyncSession=Depends(get_db),user:User=Depends(require_roles("admin", "superadmin")))->dict[str,str]:
    ...
    dispute.status,dispute.resolution,dispute.resolved_by,dispute.resolved_at="resolved",resolution,user.id,datetime.now(UTC)
