...
def upgrade() -> None:
    ...
    rule = {"minimum_fare":"2500","per_km_rate":"50","loading_base_charge":"100","loading_per_parcel":"7","unloading_base_charge":"100","unloading_per_parcel":"7","included_stops":2,"extra_stop_charge":"500","night_charge":"500","free_waiting_hours":"1","waiting_charge_per_hour":"500","range_percent":"10"}
    # PostgreSQL JSON values cannot be rendered by Alembic's offline literal
    # compiler. A quoted JSON document keeps both online and --sql upgrades
    # reproducible without relying on Python-bound parameters.
    encoded = json.dumps(rule).replace("'", "''")
    op.execute(f"INSERT INTO application_settings (key, value) VALUES ('trip_price_suggestion', '{encoded}'::json)")
